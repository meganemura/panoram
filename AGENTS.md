...
panoram answers questions about the state of one developer's machine: which coding agents run where, which repositories are dirty, which worktrees have nobody in them.
...
The usage documentation of panoram is `skills/panoram/SKILL.md` with its references; a change to a flag or a query edits it in the same commit.
